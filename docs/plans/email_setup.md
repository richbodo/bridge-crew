...
Goal: every flow that depends on email — signup confirmation, password reset, room invites, bounce handling — can be tested automatically, and the agent can exercise those flows during development without waiting on the human owner.
...
Proposed plan: MX for `globaldonut.com` on a DigitalOcean Ubuntu droplet (receiving), Postmark for sending, app at `bridge.globaldonut.com`.
...
3. **Choosing Postmark means opting out of Lovable's built-in email.** That's a legitimate trade — you get Postmark's dashboard, message search API, and webhook detail, which is genuinely better for automated testing. The cost is that the agent hand-rolls the send path and the auth-email hook instead of using scaffolded managed sending, and you own the API keys. Worth naming explicitly before building it.
   - If you'd rather not run a send integration at all, the alternative is Lovable-managed sending on `notify.globaldonut.com` plus your droplet purely for receiving. Say the word and this section can be swapped.
4. **Postmark's inbound stream can replace the IMAP layer entirely.** Postmark can receive mail too: point the test subdomain's MX at Postmark's inbound server, and every message becomes a JSON webhook plus a queryable message. That would remove the droplet from the picture, and remove IMAP polling from the tests. The droplet is still fine — and gives you full control — but it's real ops work (postfix, dovecot, TLS certs, fail2ban, disk, patching) for a testing convenience. I'd take Postmark inbound unless you specifically want the mail server.
   - This plan below assumes the droplet, since that's what was asked for. Sections marked *(skip if using Postmark inbound)* fall away if you change your mind.
...
- Specs added:
  ...
  - Suppressed recipient: sending to a known-bad address surfaces "can't receive mail" rather than silence.
...
- Specs in that project:
  1. **Signup confirmation** — sign up as `e2e-signup-<runid>@t.globaldonut.com`, click the confirm link, land signed in at the lobby.
  ...
  4. **Bounce** — send to the reserved bad address, then assert the invite row shows undeliverable via the Postmark webhook.
- Since every address is unique per run, tests never collide and the mailbox needs no cleanup from the agent. The owner can prune it on a schedule.
...
Notes: `mail` must be grey-clouded — proxying breaks SMTP. Take the exact DKIM and Return-Path values from Postmark's domain screen, not from this table.
...
4. Copy the **Server API Token** and share it privately — it will be stored as a project secret. Do not paste it in chat history.
5. Under **Webhooks**, add a webhook for Bounce, Spam Complaint, and Delivery pointing at `https://bridge.globaldonut.com/api/public/email/postmark`. Set the Basic-auth username/password fields to a random string and share that too; the endpoint verifies it.
...
5. Create the mailbox user `crewtest` with a long random password. Give the agent **that** account only; it can read the catch-all box and nothing else.
...
9. Share: IMAP host, port (993), username, password. They will be stored as secrets.

### D. Supabase auth email routing

Auth emails (confirm, reset) need to go through Postmark too, or the confirm/reset tests will exercise the default sender. The agent will wire the auth email hook to Postmark once the token is in place — no action needed from you beyond step B.

### E. One reserved bad address

Pick and tell the agent one address that will always hard-bounce, e.g. `nobody@t.globaldonut.com` configured in postfix to reject with 550. That's what the bounce test targets.
